...
export function transcriptShowdownConverter() {
	var converter = new showdown.Converter({
		strikethrough: true,
...
export function postObj(id, fileContents) {
	var converter = showdownConverter()
	var html = converter.makeHtml(fileContents)
	// var metadata = converter.getMetadata()
	const { data } = matter(fileContents)
	data.date = data.date.toString()
	const transcriptsPath = path.join(process.cwd(), "transcripts")
	const fullPathTranscript = path.join(transcriptsPath, `transcript-${id}.md`)
	if (fs.existsSync(fullPathTranscript)) {
		const transcriptContents = fs.readFileSync(fullPathTranscript, "utf8")
		if (transcriptContents) {
			var transcriptConverter = transcriptShowdownConverter()

			var transcriptHtml = transcriptConverter.makeHtml(transcriptContents)
		}
		data.transcriptHtml = transcriptHtml
	}
...
export function getSortedPostsData(type) {
	...
	const allPostsData = fileNames.map((fileName, index) => {
		...
		// const matterResult = matter(fileContents)
		// var textContent = html.replace(/<[^>]+>/g, "") || ""
		const { html, metadata, textContent } = postObj(id, fileContents)
